...
for n_algo in ['rfr','gbm']:
    # Choose and set the algorithm to run here
    #    Random Forest          = 'rfr'
    #    Gradient Boosting      = 'gbm'
    print ('   --> Algorithm:', n_algo)
    if   n_algo == 'rfr':
        fi_model = RandomForestClassifier(numTrees=1000, seed=0)

        # Class weight not avilable in ML
        # More info at http://spark.apache.org/docs/2.2.0/api/python/pyspark.ml.html
    elif n_algo == 'gbm':
        fi_model = GradientBoostingClassifier(n_estimators=1000, max_features=0.90, max_depth=6, random_state=0)
        
        # max_features not available, but maxBins exists..
        # maxIter same as n_estimator?
